...
export default function AdminControlPage() {
  ...
  // ইনপুট বক্সের জন্য কমন স্টাইল
  const inputStyle = "w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3 text-sm focus:ring-2 focus:ring-[#ff6600] outline-none transition-all";
              ...
              <div key={ad.id} className="bg-white p-5 rounded-3xl shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 border border-slate-100">
                <div className="flex items-center gap-5 w-full">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-slate-400 font-bold uppercase">₹{ad.price} | {ad.category} &gt; {ad.sub_category}</p>
                  </div>
                </div>
      ...
      {editingAd && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-4xl w-full shadow-2xl my-8">
            <h2 className="text-2xl font-black italic uppercase mb-8 text-slate-900 border-b pb-4">Edit Details</h2>
            
            <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              
              <div>
                <label className={labelStyle}>Category</label>
                <input type="text" value={editForm.category} className={inputStyle} onChange={(e) => setEditForm({...editForm, category: e.target.value})} />
              </div>

              <div>
                <label className={labelStyle}>Sub Category</label>
                <input type="text" value={editForm.sub_category} className={inputStyle} onChange={(e) => setEditForm({...editForm, sub_category: e.target.value})} />
              </div>

              <div>
                <label className={labelStyle}>Item Type</label>
                <input type="text" value={editForm.item_type} className={inputStyle} onChange={(e) => setEditForm({...editForm, item_type: e.target.value})} />
              </div>
              ...
              <div className="lg:col-span-3 flex gap-4 mt-4">
                <button type="submit" className="flex-1 bg-[#ff6600] text-white py-4 rounded-2xl font-black uppercase text-xs">Save Changes</button>
