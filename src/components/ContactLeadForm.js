  ...
  const handleSubmit = async (e) => {
    ...
    if (!error) {
      ...
      if (mode === "whatsapp" || mode === "full-whatsapp") {
        window.open(`https://wa.me/${ad.contact_number}?text=Hi, I am interested in: ${ad.title}`, '_blank');
      } else {
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input required type="text" placeholder="YOUR NAME" className="w-full bg-slate-100 border-none rounded-2xl px-5 py-4 text-sm font-bold" onChange={(e) => setFormData({...formData, name: e.target.value})}/>
              <input required type="tel" placeholder="PHONE NUMBER" className="w-full bg-slate-100 border-none rounded-2xl px-5 py-4 text-sm font-bold" onChange={(e) => setFormData({...formData, phone: e.target.value})}/>
              <button type="submit" disabled={submitting} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest">{submitting ? "SAVING..." : "SUBMIT & CONTACT"}</button>
